...
const AllButton = () => {
  ...
  const mintToken = async () => {
    try {
      ...
      const mint = await contractInstance.mint(mintAddress, ethers.utils.parseEther(mintAmount));
  ...
  const getBalanceOf = async () => {
    try {
      ...
      alert(`Balance of ${getBalanceAddress}: ${ethers.utils.formatEther(balance)} tokens`);
    } catch (error) {
  ...
  const transferTo = async () => {
    try {
      ...
      const transaction = await contractInstance.transfer(toAddress, ethers.utils.parseEther(transferAmount));
  ...
  const approve = async () => {
    try {
      ...
      const transaction = await contractInstance.approve(approveAddress, ethers.utils.parseEther(approveAmount));
      const transactionHash = await transaction.wait();
  ...
  const transferFromTo = async () => {
    try {
      ...
      alert(`Transferred ${transferFromAmount} tokens from ${transferFromAddress} to ${transferToAddress}`);
    } catch (error) {
        console.error('Error transferring tokens:', error);
    }
  };
  ...
  return (
    <div>
      ...
      <button className="btn btn-primary m-5" onClick={connectToWallet}>Connect Wallet</button>

      {address && <p className="alert alert-success text-primary">Connected Address: {address}</p>}

      {/* Get Balance */}
      <div className="form-group row d-flex align-items-center">
        <label className="col-md-2 text-right">Get Balance <span className="text-danger">*</span></label>
        <input
        ...
        />
        <button className="btn btn-primary" onClick={getBalanceOf}>Get </button>
      </div>

      {/* Mint */}
      <div className="form-group row d-flex align-items-center">
        <label className="col-md-2 text-right">Mint Address <span className="text-danger">*</span></label>
        <input
          className="form-control col-md-3 mr-4"
          type="text"
        ...
        />
        <button className="btn btn-primary" onClick={() => mintToken(mintAddress, mintAmount)}>Mint</button>
      </div>


      {/* Transfer */}
      <div className="form-group row d-flex align-items-center">
        <label className="col-md-2 text-right">Transfer To <span className="text-danger">*</span></label>
        <input
          className="form-control col-md-3 mr-4"
          type="text"
        ...
        />
        <button className="btn btn-primary" onClick={transferTo}>Transfer</button>
      </div>

      {/* Approve */}
      <div className="form-group row d-flex align-items-center">
        <label className="col-md-2 text-right">Approve Spender <span className="text-danger">*</span></label>
        <input
          className="form-control col-md-3 mr-4"
          type="text"
      ...
      {/* Transfer From */}
      <div className="form-group row d-flex align-items-center">
        <label className="col-md-2 text-right">From Address <span className="text-danger">*</span></label>
        <input
        ...
        />
        <button className="btn btn-primary" onClick={transferFromTo}>Transfer</button>
      </div>
